...
func (a *Api) GetNodeComplianceCandidateRulesets(c echo.Context, nodeId string) error {
	...
	if err != nil {
		...
		return JSONProblemf(c, 404, "NotFound", "node %s not found", nodeId)
	}
	...
	if err != nil {
		...
		return JSONProblemf(c, http.StatusInternalServerError, "InternalError", "cannot get candidate rulesets for node %s", node.NodeID)
	}
