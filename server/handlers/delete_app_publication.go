...
// DeleteAppPublication handles DELETE /apps/{app_id}/publications/{group_id}
func (a *Api) DeleteAppPublication(c echo.Context, appId string, groupId string) error {
	return a.deleteAppPublication(c, "DeleteAppPublication", appId, groupId)
}

func (a *Api) deleteAppPublication(c echo.Context, handlerName, appId, groupId string) error {
	...
	n, err := odb.DeleteAppPublication(ctx, app.ID, group.ID)
	if err != nil {
		log.Error("cannot delete publication", "app_id", appId, "group_id", groupId, logkey.Error, err)
		return JSONProblemf(c, http.StatusInternalServerError, "cannot unpublish app %s from group %s", app.App, group.Role)
	}
	if n == 0 {
		return JSONProblemf(c, http.StatusNotFound, "app %s is not published to group %s", app.App, group.Role)
	}

	msg := fmt.Sprintf("app %s unpublished from group %s", app.App, group.Role)
	userEmail, _ := c.Get(XUserEmail).(string)
	if logErr := odb.Log(ctx, cdb.LogEntry{
		Action: "apps.publication.delete",
		User:   userEmail,
		Fmt:    "app %(u)s unpublished from group %(g)s",
		Dict: map[string]any{
			"u": app.App,
			"g": group.Role,
		},
