...
func TaskTrim(ctx context.Context, task *Task, db *sql.DB) (err error) {
	errors.Join(err, TaskTrimSaves(ctx, task, db))
	errors.Join(err, TaskTrimLog(ctx, task, db))
	errors.Join(err, TaskTrimStatDayDiskArray(ctx, task, db))
	errors.Join(err, TaskTrimStatDayDiskArrayDG(ctx, task, db))
	errors.Join(err, TaskTrimStatDayDiskApp(ctx, task, db))
	errors.Join(err, TaskTrimStatDayDiskAppDG(ctx, task, db))
	errors.Join(err, TaskTrimSwitches(ctx, task, db))
	errors.Join(err, TaskTrimCompLog(ctx, task, db))
	errors.Join(err, TaskTrimCompLogDaily(ctx, task, db))
	errors.Join(err, TaskTrimResmonLog(ctx, task, db))
	errors.Join(err, TaskTrimSvcmonLog(ctx, task, db))
	errors.Join(err, TaskTrimSvcactions(ctx, task, db))
	errors.Join(err, TaskTrimDashboardEvents(ctx, task, db))
	errors.Join(err, TaskTrimPackages(ctx, task, db))
	errors.Join(err, TaskTrimPatches(ctx, task, db))
	errors.Join(err, TaskTrimNodeIP(ctx, task, db))
	errors.Join(err, TaskTrimNodeUsers(ctx, task, db))
	errors.Join(err, TaskTrimNodeGroups(ctx, task, db))
	errors.Join(err, TaskTrimCompRunRuleset(ctx, task, db))
	errors.Join(err, TaskTrimLinks(ctx, task, db))
	errors.Join(err, TaskTrimServicesLog(ctx, task, db))
	return
}
