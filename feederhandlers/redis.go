...
func (f *Feeder) pushUniqValue(ctx context.Context, key string, value string) error {
	s := fmt.Sprintf("LPOS %s %s", key, value)
	slog.Info(s)
	_, err := f.Redis.LPos(ctx, key, value, redis.LPosArgs{}).Result()
	switch err {
	case nil:
		// already in queue
		return nil
	case redis.Nil:
		...
		slog.Info(s)
		if _, err := f.Redis.LPush(ctx, key, value).Result(); err != nil {
...
func (f *Feeder) pushNotPending(ctx context.Context, pendingKey, queueKey string, value string) error {
	s := fmt.Sprintf("HGET %s %s", pendingKey, value)
	slog.Info(s)
	_, err := f.Redis.HGet(ctx, pendingKey, value).Result()
	switch err {
	case nil:
		// already in the list
		return nil
	case redis.Nil:
		...
		slog.Info(s)
		if _, err := f.Redis.HSet(ctx, pendingKey, value, value).Result(); err != nil {
			return fmt.Errorf("%s: %w", s, err)
		}
		s = fmt.Sprintf("LPUSH %s %s", queueKey, value)
		slog.Info(s)
		if _, err := f.Redis.LPush(ctx, queueKey, value).Result(); err != nil {
			return fmt.Errorf("%s: %w", s, err)
		}
		return nil
	default:
